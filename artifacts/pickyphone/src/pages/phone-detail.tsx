...
function SpecSection({ title, rows, delay }: { title: string; rows: { label: string; value: string }[]; delay: number }) {
  return <motion.div variants={fadeUp} initial="hidden" animate="show" custom={delay} className="rounded-2xl border border-border bg-card overflow-hidden"><div className="px-5 py-3 border-b border-primary/15 bg-primary/5"><h3 className="text-xs font-serif font-bold text-primary uppercase tracking-widest">{title}</h3></div><div className="divide-y divide-border/40">{rows.map(row => <div key={row.label} className="flex justify-between items-center px-5 py-3 gap-4"><span className="text-xs text-muted-foreground shrink-0">{row.label}</span><span className="text-sm text-right font-medium leading-snug">{row.value}</span></div>)}</div></motion.div>;
}
  ...
  return (
    ...
    >
      <div className="flex items-baseline justify-between mb-6">
        <div>
          <p className="text-xs font-bold tracking-widest text-primary uppercase mb-1">You Might Also Like</p>
          <h2 className="text-2xl font-serif font-bold">Similar Phones</h2>
        </div>
...
export default function PhoneDetail() {
  ...
  const scoreBar = (score: number) => <div className="flex items-center gap-2"><div className="flex-1 h-1.5 rounded-full bg-border overflow-hidden"><motion.div initial={{ width: 0 }} animate={{ width: `${score}%` }} transition={{ duration: 0.8, delay: 0.4, ease: [0.22, 1, 0.36, 1] }} className="h-full rounded-full bg-gradient-to-r from-primary/70 to-primary" /></div><span className="text-xs font-semibold text-primary w-12 text-right">{score}/100</span></div>;
  return (
    <div className="min-h-[100dvh] flex flex-col bg-background text-foreground">
      ...
      <header className="border-b border-border/40 px-5 py-4 flex items-center gap-4 sticky top-0 z-50 bg-background/80 backdrop-blur-xl"><button onClick={() => window.history.back()} className="flex items-center gap-2 text-sm text-muted-foreground hover:text-primary transition-colors"><ArrowLeft className="w-4 h-4" /><span className="hidden sm:inline">Back</span></button><Link href="/" className="font-serif text-xl font-bold tracking-tight text-primary mx-auto">PickyPhone.</Link><div className="w-20 hidden sm:block" /></header>
